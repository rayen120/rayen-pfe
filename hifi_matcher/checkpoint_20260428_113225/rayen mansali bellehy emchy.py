...
def extract_eticket_serial_number_step4_strict(image_bgr):
    ...
    def _ingest_ocr_text(txt, weight=3):
        ...
        compact = re.sub(r"[^0-9]", "", txt)
        if len(compact) >= 12:
            raw_digit_blobs.append(compact)
        if len(compact) >= 13:
    ...
    if not valid and raw_digit_blobs:
        # OCR-only repair for near-miss strings (extra/missing digit), still checksum-gated.
        for blob in raw_digit_blobs[:80]:
            b = re.sub(r"[^0-9]", "", blob or "")
            if len(b) < 12:
                continue
            repaired = set()
            if len(b) == 12:
                ...
                for i in range(len(b)):
                    repaired.add(b[:i] + b[i + 1 :])
            else:
                for i in range(0, len(b) - 14 + 1):
                    w14 = b[i : i + 14]
                    for j in range(14):
                        repaired.add(w14[:j] + w14[j + 1 :])
                for i in range(0, len(b) - 13 + 1):
                    repaired.add(b[i : i + 13])
            for cand in repaired:
                if len(cand) == 13 and cand.isdigit() and _ean13_checksum_ok(cand):
                    # Favor candidates close to the OCR blob and barcode-like GS1 prefix 3.
                    base = 6 + (2 if cand.startswith("3") else 0)
                    sim = fuzz.ratio(cand, b[:13]) if len(b) >= 13 else 0
                    scores[cand] = max(scores.get(cand, 0), base + int(sim // 10))
        valid = [(code, sc) for code, sc in scores.items() if _ean13_checksum_ok(code)]
    if not valid:
        # Final OCR-only fallback for stubborn images.
        try:
            generic = (extract_eticket_serial_number(image_bgr) or "").strip()
        except Exception:
            generic = ""
        if generic and len(generic) == 13 and generic.isdigit() and _ean13_checksum_ok(generic):
            return generic
        return ""
    ...
    def find_match(self, strict_live=False, forced_ref=""):
        ...
        def task():
            ...
            if strict_live and forced_ref_n and selected_ref_live:
                ...
                compare_ref = selected_ref_live if zero_nine else forced_ref_n
