...
OCR_MAX_SIDE = 1280
# Maquette Eticket: big reference text is almost always in the upper part of the label photo.
MAQUETTE_ETICKET_TOP_CROP = 0.45
...
def ocr_reference(image_bgr):
    ref, raw = extract_reference_tesseract_full(image_bgr)
    if ref:
        return ref, raw
    try:
        text = ocr_text_tesseract(image_bgr)
    except Exception:  # noqa: BLE001
        text = ""
    r2 = extract_reference_from_text(text)
    return (r2, text) if r2 else ("", text)
...
class HifiMatcherApp:
    ...
    def _migrate_clients_schema(self, conn):
        ...
        if "maquette_eticket_path" not in cols and "maquette_photo_path" in cols:
            try:
                ...
                conn.execute(
                    """
                    INSERT INTO clients_mig (id, name, maquette_eticket_path, maquette_filtre_path, filter_reference_code, date_added)
    ...
    def _build_matcher_ui(self, parent):
        ...
        self.set_status("Select both folders to start.")
    ...
    def _build_client_db_ui(self, parent):
        ...
        btn_row.grid(row=4, column=0, columnspan=4, sticky=tk.W, padx=5, pady=8)
    ...
    def _validate_client_form_core(self):
        ...
        if filtre and not os.path.isfile(filtre):
            ...
            return None
        return name, eticket, filtre or "", ref
    ...
    def save_client(self):
        ...
        try:
            cursor = conn.execute(
                """
                INSERT INTO clients (name, maquette_eticket_path, maquette_filtre_path, filter_reference_code)
                VALUES (?, ?, ?, ?)
                """,
                (name, eticket, filtre, ref),
            )
    ...
    def refresh_client_list(self, query=""):
        ...
        try:
            if query:
                ...
                cur = conn.execute(
                    """
                    SELECT id, name, maquette_eticket_path, maquette_filtre_path, filter_reference_code, date_added
                    FROM clients
        ...
        for client in self.clients:
            client_id, name, et_path, fi_path, ref, date_added = client
    ...
    def _build_client_auto_pipeline(self, filter_ref, selected_eticket_ref, client_row):
        """
        ...
        if not client_row:
            return "", None
        _cid, cname, mq_et_path, mq_fi_path, stored_filter_ref, _date = client_row
        ...
        all_known = bool(sel and mq_et_ref and fr and mq_fi_ref)
        all_ok = bool(all_known and step1_ok and step2_ok and step3_ok)
        if all_known:
    ...
    def on_client_select(self, _event=None):
        ...
        if not row:
            return
        client_id, name, eticket, filtre, ref, _date_added = row
    ...
    def update_client(self):
        ...
        try:
            conn.execute(
                """
                UPDATE clients SET name=?, maquette_eticket_path=?, maquette_filtre_path=?, filter_reference_code=?
                WHERE id=?
                """,
                (name, eticket, filtre, ref, row[0]),
            )
            conn.commit()
        except sqlite3.IntegrityError:
            messagebox.showerror("Duplicate Client", "A client with this name already exists. Please use a unique name.")
    ...
    def delete_client(self):
        ...
        if not row:
            ...
            return
        client_id, name, _et, _fi, _ref, _date_added = row
    ...
    def verify_selected_client_with_eticket(self):
        ...
        _client_id, name, eticket_path, _filtre_path, ref, _date_added = row
    ...
    def verify_selected_client_with_filter(self):
        ...
        _client_id, name, _eticket_path, maquette_filtre_path, client_ref, _date_added = row
    ...
    def _draw_ocr_overlay(self, frame_bgr, ocr_fps):
        ...
        for words in line_groups.values():
            ...
            if line_ref in KNOWN_REFERENCE_SET:
        ...
        for x, y, w, h, txt, cleaned in word_rows:
            ...
            # For non-confirmed refs, keep subtle red box without noisy text labels.
            color = (0, 255, 0) if is_known_ref else (0, 0, 255)
            cv2.rectangle(vis, (x, y), (x + w, y + h), color, 2)
            if is_ref:
                label = word_ref
            else:
                parsed = _live_ref_from_text_fast(compact_txt) or _best_known_ref_from_text_confusion(compact_txt, min_score=90)
            ...
            if label:
                cv2.putText(
                    ...
                    (0, 0, 0),
                    (lbl_thick + 2) if is_known_ref else (lbl_thick + 1),
                    cv2.LINE_AA,
                )
                cv2.putText(
                    vis,
                    ...
                    (lbl_scale if is_ref else max(0.5, lbl_scale - 0.08)),
                    color,
                    lbl_thick if is_known_ref else 1,
                    cv2.LINE_AA,
                )
        if ref_votes:
            best_known_ref = max(ref_votes.items(), key=lambda kv: kv[1])[0]
        h_img, _ = vis.shape[:2]
        cv2.putText(
            vis,
            f"OCR FPS: {ocr_fps:.1f}" if ocr_fps > 0 else "OCR FPS: --",
        ...
        if ok and frame is not None and frame.size > 0:
            ...
            if os.path.isfile(out_path):
                ...
                if (
                    ...
                    and (now - float(getattr(self, "live_last_match_ts", 0.0) or 0.0) >= 2.2)
                ):
                    ...
                    if selected_ref:
                        exact_now = overlay_ref == selected_ref
                        fast_score = eticket_compatibility_vs_filter(overlay_ref, selected_ref)
                        fast_msg = (
                        ...
                        if (not exact_now) and self._is_zero_nine_confusion_pair(overlay_ref, selected_ref):
                            fast_msg += "\nPossible OCR confusion: 0 <-> 9"
                        self.set_status(fast_msg)
                    else:
                        self.set_status(
                            f"🟢 Reference confirmed: {overlay_ref}\n"
                            "Live camera stopped.\n"
    ...
    def _start_live_strict_worker(self, frame_bgr, need):
        ...
        def _worker():
            ...
            try:
                self.root.after(
                    0,
                    lambda: self._apply_live_strict_result(strict_ref, why, int(need)),
                )
            except Exception:
                self.live_strict_busy = False
    ...
    def _apply_live_strict_result(self, strict_ref, why, need):
        self.live_strict_busy = False
        if not self.live_camera_mode:
            return
        now = time.monotonic()
    ...
    def _live_get_strict_ref(self, frame_bgr):
        ...
        try:
            ...
            if roi is not None and roi.size > 0:
                ...
                if quick_hits:
                    ...
                    if quick_best not in KNOWN_REFERENCE_SET:
                        snapped = closest_known_reference(quick_best, min_score=90)
                        if snapped:
                            quick_best = normalize_reference(snapped)
        ...
        if explicit_refs:
            ...
            if exp_best in KNOWN_REFERENCE_SET:
                return exp_best, "explicit_strict_token"
            snapped_exp = closest_known_reference(exp_best, min_score=94)
            if snapped_exp and snapped_exp in KNOWN_REFERENCE_SET:
                return normalize_reference(snapped_exp), "explicit_strict_token_snapped"
        ...
        # If candidate is not directly indexed, try conservative snap.
        if best_ref not in KNOWN_REFERENCE_SET:
            snapped = closest_known_reference(best_ref, min_score=92)
            if snapped:
                best_ref = normalize_reference(snapped)
    ...
    def find_match(self, strict_live=False, forced_ref=""):
        ...
        def task():
            ...
            if strict_live and forced_ref_n and selected_ref_live:
                fast_score = eticket_compatibility_vs_filter(forced_ref_n, selected_ref_live)
                zero_nine = self._is_zero_nine_confusion_pair(forced_ref_n, selected_ref_live)
                ...
                return {
                    ...
                    "filter_ref": forced_ref_n,
                    "selected_ref": selected_ref_live,
                    "selected_exact": bool(forced_ref_n == selected_ref_live),
                    ...
                    "raw_text": f"Live forced reference: {forced_ref_n}",
                }
            if manual_ref:
        ...
        def on_success(result):
            ...
            def _status_with_compare(msg, tone="neutral"):
                ...
                self.set_status(msg + self._matcher_eticket_compare_footer(fr), tone=tone)
            ...
            if result.get("type") == "selected_compare_fast":
                ...
                if result.get("zero_nine_hint"):
                    msg += "Possible OCR confusion: 0 <-> 9\n"
